...
$Punten = $_POST["PuntenArenas"];
$date = $_POST["dateArenas"];

// select info from db where date is $date
$sql = 'SELECT * FROM `puntenarenas` WHERE Datum = "'.$date.'" AND `Account` = '. $_SESSION["ID"] .'';
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
    ...
    $sql = "UPDATE puntenarenas SET punten='".$Punten."' WHERE `Datum`='".$date."' AND `Account` = ". $_SESSION["ID"] ."";
  ...
  $sql = "INSERT INTO puntenarenas (Punten, Datum, Account)
  VALUES (".$Punten.", '".$date."', ". $_SESSION["ID"] .")";
  // echo it back to admin page
  if ($conn->query($sql) === TRUE) {
      echo '{"Punten": '. $Punten .'}';
  } else {
      echo "{2Error inserting record: " . $conn->error ."}";
  }
}
